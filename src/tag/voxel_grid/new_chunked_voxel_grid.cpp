...
namespace tag {
    namespace voxel_grid {
        const block_instance& new_chunked_voxel_grid::get_block(const world_coord& coord_world) const {
        ...
        block_instance& new_chunked_voxel_grid::get_block(const world_coord& coord_world){
            auto chunk_coord_pair = from_world_coord(coord_world);
            auto chunk = get_data_chunk(chunk_coord_pair.first);
            return chunk.get_block(chunk_coord_pair.second);
        }

        void new_chunked_voxel_grid::set_block(block_instance block, const world_coord& coord_world){
            auto chunk_coord_pair = from_world_coord(coord_world);
            auto chunk = get_data_chunk(chunk_coord_pair.first);
        ...
        std::vector<new_display_chunk> new_chunked_voxel_grid::get_display_chunks(std::shared_ptr<util::registry<tag::block_type>> block_registry, const world_coord& coord_world, int range) const{
            ...
            std::vector<new_display_chunk> chunks_in_range;
            for(auto coord : chunk_range){
                ...
                if(display_chunk_search != display_chunk_cache.end()){
                    ...
                    new_display_chunk fresh_display_chunk(block_registry, coord, chunk);
        ...
        new_data_chunk& new_chunked_voxel_grid::get_data_chunk(const chunk_coord& coord) const{
